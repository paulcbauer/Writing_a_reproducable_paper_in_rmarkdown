...
linestretch: 1.2
colorlinks: true
abstract: \noindent\setstretch{1}The present paper provides a template for a reproducable scientific paper written in R Markdown. Below I outline some of the "tricks"/code (e.g., referencing tables, sections etc.) I had to figure out to produce this document. The underlying files which produce this document can be downloaded [here](https://drive.google.com/drive/folders/1zJP3cNPrHN-gj0rcmbHQgg-XA0hqDXdd?usp=sharing). I think I got pretty far but there is always room for improvement and more automatization, in parallel to the incredible developments in R and Rstudio (bookdown etc.). My last paper is almost entirely based on R Markdown [see @Bauer2018-hd and [reproduction files](https://dataverse.harvard.edu/dataset.xhtml?persistentId=doi:10.7910/DVN/DUMGLT)]. I intend to update this file when I discover more convenient code.\vspace{.8cm}
...
install.packages(c("rmarkdown", "knitr", "kableExtra",
                   ...
                   "bookdown"))
```
* ...download the 4 input files I created (see below) from [this folder](https://drive.google.com/drive/folders/1zJP3cNPrHN-gj0rcmbHQgg-XA0hqDXdd?usp=sharing).
...
All the files you need to produce the present PDF file are 4 input files...
...
I would recommend to use the command below and copy/paste the output into the first R chunk in your `paper.rmd` file. Normally we would do this when we compile/generate the paper for the last time. For more advanced tools see [packrat](https://rstudio.github.io/packrat/).
...
Table \@ref(tab-summarystats) shows summary stats of your data. I normally use `stargazer()` [@hlavac2013stargazer] which offers extreme flexibility regarding table output (see `?stargazer`). 
...
Table \@ref(tab-regression) shows the output for a regression table. Make sure you name all your models and explicitly refer to model names (M1, M2 etc.) in the text.
...
Another great function is `kable()` (`knitr` package) in combination with `kableExtra`. Table \@ref(tab:tab-kabletable) provides an example. Again you can modify so many things in both the `kable()` and the `kable_styling()` function. See [this overview](https://haozhu233.github.io/kableExtra/awesome_table_in_pdf.pdf) of all the kable stylings that are possible provided by the package author himself.

```{r tab-kabletable, echo=TRUE, message=FALSE, warning=FALSE, paged.print=FALSE, results="asis"}
library(knitr)
library(kableExtra)
kable(cars[1:10,], row.names = TRUE, caption = 'Table with kable() and kablestyling()', 
...
Inline code/results that depend on earlier objects in your document will automatically be updated once you change those objects. For instance, imagine a reviewer asks you to omit certain observations from your sample. You can simply do so in the beginning of your code and push play subsequently.. at time you might have to set `cache = FALSE` at the beginning so that all the code chungs are rerun.    
Researchers often avoid referring to results intext etc. because you easily forget to change them when revising a manuscript. At the same it can make an article much more informative and easier to read, e.g., if you discuss a coefficient in the text you can directly show it in the section in which you discuss it. Inline code allows you to do just that. R Markdown allows you to that do so in a reproducible and automatized manner.
...
## Plotly graphs
Plotly is a popular graph engine that let's you also produce interactive graphs that you can embed in html webpages or documents (e.g., see [here](https://paulcbauer.shinyapps.io/visualizing-causal-scenarios/)). I am a big fan, however, right now there is no easy, automatic way to insert high resolution Plotly graphs into your R Markdown PDF. See Figure \@ref(fig:fig-3) below.      
There seems to be a new engine for exporting graphs locally without relying on the plotly server --- which you wanna avoid for a properly reproducible PDF --- called orca (`?orca`). However, the [installation](https://github.com/plotly/orca#installation) is too challenging for the average user at the moment.

```{r fig-3, fig.align="center", fig.cap="Scatterplot of Speed and Distance", fig.pos="H", message=FALSE, warning=FALSE}
library(plotly)
plot_ly(cars, type = "scatter", mode="markers",
        x=~speed, 
        y=~dist)
```
...
* Use document the document outline in R studio (Ctrl + Shift + O) when you work with R Markdown.
...
* Use "really" informative variable names:
    + Q: What do you think does the variable *trstep* measure? It actually measures trust in the European parliament. 
        + How could we call this variable instead? Yes, `trust.european.parliament` which is longer but will probably be understood by one of your descendants in 100 years.
...
The github directory contains a wide variety of citation style files depending on what discipline your work in.
