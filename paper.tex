...
\item
  ...
  but why not aim for reproducability in 500 years.
\item
...
Table \ref{tab-summarystats} shows summary stats of your data. I
normally use \texttt{stargazer()} (Hlavac
...
\KeywordTok{stargazer}\NormalTok{(cars, }
          ...
          \DataTypeTok{label=}\StringTok{"tab-summarystats"}\NormalTok{, }
...
Table \ref{tab-regression} shows the output for a regression table. Make
sure you name all your models and explicitly refer to model names (M1,
M2 etc.) in the text.
...
\KeywordTok{stargazer}\NormalTok{(model1, model2, model3,}
          ...
          \DataTypeTok{label=}\StringTok{"tab-regression"}\NormalTok{, }
          \DataTypeTok{table.placement =} \StringTok{"H"}\NormalTok{, }
...
combination with \texttt{kableExtra}. Table \ref{tab:tab-kabletable}
provides an example. Again you can modify so many things in both the
\texttt{kable()} and the \texttt{kable\_styling()} function. See
...
\KeywordTok{kable}\NormalTok{(cars[}\DecValTok{1}\OperatorTok{:}\DecValTok{10}\NormalTok{,], }\DataTypeTok{row.names =} \OtherTok{TRUE}\NormalTok{, }\DataTypeTok{caption =} \StringTok{'Table with kable() and kablestyling()'}\NormalTok{, }
...
\caption{\label{tab:tab-kabletable}Table with kable() and kablestyling()}
\centering
\resizebox{\linewidth}{!}{
